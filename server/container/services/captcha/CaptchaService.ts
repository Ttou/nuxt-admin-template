...
export class CaptchaService {
  ...
  async verify(captchaId: string, captchaValue: string | number, caseSensitive = false) {
    ...
    const cacheCaptchaValue = await this.cacheManager.get<string | number>(cacheKey)
  ...
  private async save(captchaId: string, captchaValue: string | number) {
    await this.cacheManager.set(this.getCacheKey(captchaId), captchaValue, 3 * 60 * 1000)
  }
